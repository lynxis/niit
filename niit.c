...
*/
    ...
    PDEBUG("niit : before nf_reset mac->len  %x \n", skb->mac_len);
    PDEBUG("niit : before nf_reset data->len  %x \n", skb->data_len);
	...
	tunnel->recursion--;
        ...
        if(iph6->daddr.s6_addr32[0] != tunnel->ipv6prefix_1 ||
             iph6->daddr.s6_addr32[1] != tunnel->ipv6prefix_2 ||
